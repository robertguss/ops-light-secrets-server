...
execution: code
supersedes: 2026-07-15-001-feat-ops-light-secrets-server-plan.md
---

<!--
Fifth revision (2026-07-16): the wayfinder walk
(docs/wayfinder/map.md, tickets T01-T13) re-derived every load-bearing
decision. Verdicts: nearly everything kept as written; KTD16 blind query
index and segmented audit retention/archive/prune deferred post-v0.1
(T08); the BUSL Vault-CLI shim demoted to post-v0.1 evidence-gated
(T03); KTD2's spike gate narrowed to store-facts (T04); license changed
to MIT (T13). Detail in docs/plan-history.md and the ticket files.
-->
...
- **Blocking design gates:** Rotation semantics, auth surface, storage,
  transport, and audit-chain design are settled in the Planning Contract; OQ1
  (fnox-native protocol) is answered — phase 2 gates on a use-case-first design
  discussion with jdx — and the project name is parked with a
  before-any-public-artifact deadline (OQ3). Neither touches v0.1. Four design
  gates must close before the persistent format and the final data-plane router
  are frozen — they affect persistent formats and security boundaries, not
  ordinary implementation detail:
  - G0 — U0 freezes the versioned client contract and client prerequisites.
  - G1 — U2 proves atomic multi-table commit, crash recovery, logical snapshot,
    and the measured durability envelope via the store spike; executor
    backpressure is proven by U2/U6 executor tests (moved out of the spike —
    wayfinder T04).
  - G2 — U2/U6 freeze the authenticated clear-record format, canonical crypto
    encoding, and audit event schema. (The audit query-index format left this
    gate when KTD16 was deferred post-v0.1; the index is rebuildable from
    primary events, so its format is never load-bearing.)
  - G3 — U10 freezes the logical backup format and restore/audit-epoch rules. U1
    configuration, CLI scaffolding, and transport work may proceed before these
    gates; storage-format fixtures, the final API router, and public
...
- F4. Operator rotates a shared key
  ...
  - **Steps:** The operator enumerates the secret's consumers — the declared
    ...
    status view report each snapshotted consumer as on-current, on-prior, or
    silent-since-write from the versioned read audit (R33) — verification by
    observation, not guesswork; revokes the old credential in the upstream SaaS
    product — an action the server records but cannot perform; marks the
    rotation complete, which succeeds only while the written version is still
    current and every snapshotted consumer is observed on-current — or with an
    explicit audited acknowledgement naming why an unverified consumer is
    acceptable (R33) — and the old version is retired, closing with a redacted
    closeout report.
...
- R1. The server serves the Vault KV v2 read, write, list,
  ...
  endpoint absent from the contract tests. Endpoints outside it (PATCH included)
  are rejected explicitly (R3), never silently approximated as ordinary writes.
...
- R4. Secret values and confidential audit payloads are encrypted at rest under
  ...
  the authenticated structural records, so removal or rollback to an older,
  formerly valid record is detectable through the anchored sequence; the
...
- R39. Every security-relevant clear record — identities, grants, credential
  ...
  keyed MAC under the keyring's metadata-integrity key (KTD15). Reads verify the
  MAC before using the record; a MAC failure takes readiness false and prevents
  data-plane admission — records are never accepted on a warning-only path. Each
  signed checkpoint contains a state digest over a consistent snapshot of these
  authenticated records, and `audit verify --state` verifies the anchored state
  and reconciles post-checkpoint mutations from the audit tail.
...
- R8. An identity's access is scoped to a subset of secret paths rather than the
  ...
  framework-decoded path value (KTD9). Grants are exact-path or segment-prefix
  subtree matches over that canonical form — no globs, no regular expressions,
  no deny rules, no policy language.
...
- R22. Authorization uses a closed, versioned capability set rather than one
  ...
  write. Management capabilities are likewise distinct: identity and grant
  management, credential issue/revoke, rotation management, consumer enumeration
  (R10), audit read/export, backup, key rotation, and diagnostics — so an
  audit-pulling integration needs no power to mint credentials. Built-in bundles
  (e.g. `admin`, `auditor`) are convenience aliases over the fixed set, not a
  policy language. Audit records are confidential at rest and accessible for
  query, export, and verification only under the audit capabilities.
...
- R11. A secret retains prior versions when a new value is written, and writes
  ...
  version without writing a secret; `rotation cutover` writes the target version
  and transitions the rotation from prepared to cutover atomically under the
  saved check-and-set expectation — a CAS conflict leaves the rotation prepared
  and requires an explicit refresh, cancellation, or restart, never a silently
  changed snapshot. Completing a rotation succeeds only while that target is
  still the current version and only under R33's closeout guard. Cancellation is
  permitted only before cutover; after cutover, recovery is an explicit
  copy-forward rollback into a new higher version — the current-version pointer
  never moves backward — and a superseding write marks the rotation superseded
  and prevents completion. The staging window lives upstream: the old SaaS
  credential stays valid until the operator revokes it after cutover.
- R12. The server surfaces a secret's age — time since its last completed
  ...
  interval stored in server-owned rotation metadata (KTD11) writable only
  through the local control plane, and reports "no interval set" when none is
  configured. Vault `custom_metadata` remains client-owned compatibility data
  and cannot alter rotation policy: the control-plane view may project the
  interval alongside custom metadata, but the remote metadata endpoint cannot
  overwrite or delete it. A value written outside a rotation is labeled
  `changed_since_last_completed_rotation`; the age display never implies the
  current value was covered by an older completed rotation.
- R23. R10's observed view is computed over a documented lookback window — the
  authorized view does not expire — by scanning primary audit events in the
  active store (KTD16's query index is deferred post-v0.1), and R13's audit
  retention is at least as long as that window. In v0.1 that retention bound
  holds trivially: all events are retained (segmentation and pruning deferred —
  Deferred / Open Questions). When segmented retention arrives, events inside
  the consumer-observation window are never eligible for archival pruning (R13),
  and increasing the lookback first verifies that sufficient active or queryable
  history exists.
- R29. Deleting a secret version is reversible and distinct from destruction; a
  ...
  unsupported on the remote listener in v0.1 unless U0 proves a supported client
  requires it and a separate safety decision revises this contract. Destruction
  is logical, not cryptographic erasure — historical backups, copy-on-write
  pages, and unreclaimed filesystem blocks may retain encrypted copies until
  backup expiry, compaction, or media replacement, and the documentation says so
  plainly; per-version cryptographic deletion would require a per-version
  wrapped-DEK hierarchy and is explicitly out of v0.1 (Threat Model). Per-secret
  and mount-default `max_versions` bound retained history; retention never
  destroys the prior version of a rotation that has not been marked complete — a
  protected prior version may temporarily cause the path to exceed
  `max_versions`, metadata and `doctor` report `retention_deferred_by_rotation`,
  and closeout, cancellation, or supersession immediately re-evaluates pruning —
  and behavior for retired and deleted versions is documented.
- R33. After a replacement write, a management-gated rotation status view
  ...
  (`on-prior`), or nothing since the write (`silent-since-write`) — computed by
  scanning the authenticated R13 events in the active store over R23's lookback
  window (KTD16's blind index is deferred post-v0.1; the primary events are
  always authoritative), at declared-consumer, identity, and stable
  ...
  of a replica: tokens issued through AppRole inherit the `consumer_instance_id`
  of the secret_id that created them, and workload secret_ids require an
  instance id unless the operator explicitly accepts identity-only tracking.
  Status language is "fetched version N", never "using version N" — only an
  application-level acknowledgement could prove use, and that is out of v0.1.
  Completion is guarded: `rotation complete` succeeds only when every consumer
  in the snapshot — evaluated over the snapshot's declared consumers (R40) and
  active consumer instances — is `on-current`, or with an explicit
  `--acknowledge-unverified` flag plus an audited reason; expired or revoked
  instances are labeled `retired-without-proof` and require a recorded
  retirement reason or that same override — they never silently disappear. (The
  former index-health closeout clause left v0.1 with KTD16's deferral: the guard
  evaluates primary events directly, which are always authoritative.) Either
...
- R40. The local control plane maintains a minimal declared-consumer registry. A
  record contains a stable consumer id, canonical secret resource, owner,
  ...
  always report three separately labeled sets: **declared** — registry records;
  **authorized** — identities whose current grants allow read (R10); and
  **observed** — verified audit reads inside the lookback window (R10, R23).
  ...
  covered by R33's audited override.
...
- R13. Every read, write, delete, authorization decision, and admitted
  ...
  credential material. In v0.1 every event lives in the active database — no
  segmentation, archival, or pruning ships (deferred post-v0.1; growth at
  small-team rates is a few hundred MB/year, and R27's capacity thresholds fail
  closed long before exhaustion) — so no event is ever discarded, silently or
  otherwise. When segmented retention arrives (Deferred / Open Questions), older
  closed segments may be replaced only by an authenticated archive manifest
  after the encrypted segment has been exported, verified, independently
  checkpointed, and passed the retention guard (R23, KTD4) — no event is
  silently discarded. Requests dropped by the pre-verifier rate limiter are
...
- R14. The audit log is tamper-evident at two documented assurance levels: local
  ...
  detects wholesale re-chaining and truncation through the anchored sequence. In
  v0.1 — all events active, no closed segments — verification reports two tiers:
  fully inspected events anchored by a retained checkpoint, and the unanchored
  active tail. The closed-segment manifest (audit epoch, sequence range,
  predecessor hash, final hash, event count, encrypted archive digest, state
  digest where applicable, checkpoint digest, archive format version) and its
  third tier — segments whose integrity is anchored but whose archive was not
  supplied — arrive with segmented retention (Deferred / Open Questions). The
  claim excludes an attacker controlling the running process before entries are
  anchored — the daemon holds no checkpoint private key (KTD4), but a live
...
- R27. Unauthenticated surfaces enforce request-size and attempt-rate limits
  ...
  documented worst-case checkpoint, diagnostic, and orderly-shutdown
  transaction; the data plane stops admitting before consuming it, and releasing
  or recreating the reserve is a local, audited recovery operation — ordinary
  request handling cannot spend it.
...
- R17. The first management credential is generated by local `init`, shown once
  to a TTY or caller-supplied file descriptor, and stored only as a verifier. It
  carries audience `control` and a short bootstrap TTL: the data-plane
  authenticator rejects it cryptographically before grant evaluation (R24), and
  it cannot be converted into a data-plane token. Disclosure is failure-safe and
  ordered before durability: `init` opens and validates the output sink first,
  generates the credential, writes and flushes it to that sink, and only then
  commits the initialization transaction containing its verifier — a crash or
  broken pipe before commit leaves no initialized store and a harmless disclosed
  string (`init` reruns cleanly); a crash after commit leaves an operator
  already holding the credential. A failed sink write aborts initialization. Its
...
- R18. The server refuses to start in an unsafe configuration rather than
  ...
  tolerance takes readiness false before the anomalous value is persisted (R36,
...
- R32. A single operator can take an encrypted, transactionally consistent
  ...
  unchanged, authenticated clear-record bytes, the current keyring envelope, all
  audit events (archived-segment manifests join this list if segmented retention
  ever ships — Deferred), per-table counts and digests, and a signed manifest
  (store id, schema version, keyring generation, chain head, latest included
  checkpoint, checksums). Backup creation rewraps a recovery copy of the keyring
  to the configured backup recipients and records their non-secret recipient
  fingerprints in the manifest, so a fresh-host restore needs the archive
  recipient identity and can rewrap the recovered keyring to a new live server
  recipient before activation — it does not require the failed host's active
  identity. The archive never contains a private `age` identity or the
  checkpoint signing key; the documented recovery package pairs the archive with
  offline copies of those keys, and the docs state plainly that a store backup
  without them is not a recovery. Restore constructs a new database in a
  temporary sibling of the empty target directory, verifies every frame, record
  MAC, ciphertext, audit relationship, schema, and count, fsyncs the database
  and directory, and atomically installs it only after all verification succeeds
  — and, because a restored snapshot could otherwise resurrect credentials
  revoked after it was taken, atomically increments `current_credential_epoch`:
  every restored credential record retains its original `issue_epoch`, so all
  pre-restore tokens and secret_ids fail epoch validation (R24) even where their
  verifier and TTL would otherwise pass. Identities, grants, secrets, and audit
  history survive; bearer credentials are reissued. A restore whose audit head
  predates a supplied independently retained checkpoint is a rollback recovery:
  it requires an explicit local `--start-recovery-epoch` flag and reason,
  increments both the credential epoch and the audit epoch, and opens the new
  epoch with a first event recording the restored head, the missing anchored
  range, the supplied checkpoint digest, the archive digest, the actor, and the
  reason — future checkpoints identify the new epoch, and verification reports
  the historical anchored epoch and the recovery fork separately, never
  presenting the chain as continuous (KTD17). `backup verify --full` is the
  standing disaster-recovery rehearsal: restore into an isolated temporary
  directory, validate schema and tables, authenticate and decrypt every
  encrypted record without emitting any plaintext, verify the audit chain and
  checkpoint relationship, then remove the temporary store — with the last
  success recorded so `doctor` (U12) reports its age. A backup that has never
  survived a rehearsal is treated as unproven, and the docs say so.
- R34. The remote listener exposes only the declared Vault-compatible data
  ...
  never remotely. On Linux, each accepted control connection must also present a
  kernel-reported peer uid (`SO_PEERCRED`) equal to the configured service owner
  or an explicitly allowed administrative uid; socket mode bits, the peer-uid
  check, and the control credential are all required — none substitutes for
  another.
- R35. The store carries an explicit schema/format version and a lifecycle state
  (`ready` | `reencrypting` | `restoring` | `migrating` | `compacting`). `serve`
  requires `lifecycle=ready` and refuses schema versions newer than the binary;
  migrations are forward-only, take a verified backup first, record the
  transition in the audit log, and CI retains a restore-and-migrate fixture for
  every released schema. Binary rollback after a completed migration stays
  deferred; the version stamp is what makes that deferral safe.
...
- R41. `credential epoch rotate --reason <...>` ships in v0.1 as a local
  ...
  disclosure-before-commit ordering. It reuses R32's epoch primitive — one local
  command ends every remotely usable credential.
...
- The fnox-native protocol (phase 2). OQ1 is answered (fnox discussion #615):
  jdx is open but uncommitted and wants a use-case-first design discussion —
  phase 2 now gates on that discussion, not on whether to ask.
...
- The first post-v0.1 package (v0.2): automated discovery, import, and richer
  ...
  reference discovery and import (`.env`, fnox, live Vault/OpenBao; reuse fnox's
  own scan/import machinery rather than duplicating parsers; imports stream
  through the normal write API with create-only CAS and a redacted, resumable
  result manifest — never an intermediate plaintext export), plus
...
- Post-v0.1 automation at the edges (v0.3, evidence-driven): narrow OIDC
  ...
  contracts (opt-in per secret, local-plane-configured, required field names and
  primitive JSON types only, enforced before the transactional write, audited
  override — never regexes, scripts, or a schema language); a deterministic
  `scope dry-run` advisor (advisory only — non-observation is not non-need,
  nothing is ever auto-revoked); aggregate local metrics (control-socket or
  loopback only, no path, identity, or accessor labels — R25); operator-side
  rotation adapters (an explicitly invoked CLI runs the adapter under the
  operator's authority and pipes the new value into the normal CAS write path —
  the server process never executes anything); a signed, redacted local event
  outbox (owner-only spool directory, operator-owned delivery) as the
  composition seam generic webhooks would otherwise claim; local-only encrypted
  operator runbook metadata (upstream console URLs, account identifiers) kept
  out of remote-API-visible `custom_metadata`; declarative management kept to
  non-secret `export`/`validate`/`diff` and idempotent `ensure` over concrete
  objects — omission never means deletion; and CLI sugar for short-lived
  delegated handoffs (ephemeral identity, exact-path grant, short-TTL token,
  optional use count minted atomically in one command — never a second
  authorization model or a special one-time secret type).
- Audit scale machinery (deferred 2026-07-16, wayfinder T08 — the walk's first
  real cut): KTD16's blind query index (keyed tags, verify-against-primary,
  fail-closed health, `audit index rebuild`) and segmented audit
  retention/archive/prune (R13/R14/R23's closed-segment manifests and guards).
  v0.1 rotation-status queries scan primary audit events inside R23's lookback
  window, and every audit event stays in the active database. Both are
  performance machinery closing zero threat-model failure modes at small-team
  scale, and both have clean upgrade paths over append-only data: the index
  rebuilds from primary events via `audit index rebuild` when a measured status
  scan actually hurts; segmentation arrives as an R35 forward-only schema
  migration when `doctor` shows real capacity pressure.
- The Vault-CLI shim for BUSL avoidance — demoted to post-v0.1, evidence-gated
  (wayfinder T03). The v0.1 exit ladder, cheapest first: (1) point fnox at the
  `bao` CLI (OpenBao's CLI is a vault-CLI fork; zero code — U0 proves or kills
  this rung), (2) contribute a direct-HTTP provider upstream to fnox — confirmed
  welcome by jdx
  ([fnox discussion #615](https://github.com/jdx/fnox/discussions/615)): replace
  the existing `vault` provider's CLI-backed internals in place, preserving
  current configuration, auth, and TLS behavior, with compatibility tests
  against both Vault and OpenBao, (3) document `vault` CLI installation with an
  honest licensing note — never claiming a fully-OSI toolchain while it is the
  only path. The shim is built only if all three rungs fail AND someone is
  actually blocked: it is a second product (argument parsing, output emulation,
  releases, tests), permanent once setup docs depend on it, and closes zero
  threat-model failure modes. If ever built: a tiny separate shim binary
  covering only the captured command subset — CLI emulation never lands inside
  the server daemon.
...
- **fnox transport behavior is versioned evidence, not a permanent assumption.**
  ...
  clients. The BUSL exit ladder (Scope Boundaries, U0) plans to retire that
  install requirement: `bao` CLI first, upstream direct-HTTP fnox provider
  second (jdx confirmed the PR is welcome — fnox discussion #615), documented
  `vault` install with an honest licensing note as the floor.
...
- **Compatibility is validated against a documented, pinned client matrix** —
  ...
  the latest stable OpenBao CLI, and the two most recent fnox releases, with one
  legacy Vault line retained only for a documented deployed consumer or a
...
Both are settled or parked; nothing blocks v0.1.

- OQ1. **Answered 2026-07-15** (wayfinder T02 —
  [fnox discussion #615](https://github.com/jdx/fnox/discussions/615)). jdx's
  reply: a direct-HTTP rewrite of the existing `vault` provider is welcome —
  replace the CLI-backed internals in place, preserve current configuration,
  auth, and TLS behavior, include compatibility tests against both Vault and
  OpenBao. A fnox-native protocol: interest without commitment; he wants a
  use-case-first design discussion before any decision. Phase 2 stays
  design-discussion-gated, exactly as planned; the provider rung of the BUSL
  exit ladder (Scope Boundaries) is confirmed viable.
- OQ3. Project name — **parked with a deadline** (wayfinder T13, Robert's call).
  The working name `ops-light-secrets-server` serves through the build; a real
  name must be chosen before any public artifact (announcement, crates.io
  publish, or README positioning aimed at outside readers). Renames are cheap
...
- `docs/plans/revisions/gpt.md` — the 2026-07-15 fourth-round implementation
  ...
  anchors it carries: axum `Path` percent-decoding behavior and `axum::serve`
  Unix-listener support, `tower-http` sensitive-headers, redb single-writer
  concurrency and MVCC reads, the RustCrypto `chacha20poly1305` NCC audit note
  (and `aes-gcm-siv`'s unaudited status), `SO_PEERCRED` (unix(7)), OpenBao KV v2
  metadata-delete semantics, and the current release lines at review time (Vault
  2.0.x, OpenBao 2.6, fnox 1.30).
...
`docs/plan-history.md`. The contract below is present-tense and canonical; where
a historical rationale remains load-bearing, it is stated inline in the relevant
Key Decision or KTD. Where history and this contract disagree, this contract
wins.
...
- A live attacker with root, kernel, or debugger control of the running,
  ...
  checkpoint private key itself lives only in short-lived CLI operations, KTD4).
...
- Disclosure of structural metadata to whoever holds the data directory or a
  ...
  in v0.1 (R4, KTD11). Values, credential material, and audit payloads — which
  ...
  until a post-v0.1 opaque-identifier option exists. (KTD16's blind-index
  tag-cardinality leak left this list with the index's post-v0.1 deferral; it
  returns as a consideration if the index does.)
...
- KTD1. **HTTP surface: two `axum` routers in one process.** The remote
  ...
  adopted after U9 proves TLS reload and graceful shutdown; the owner-restricted
  control plane binds a `tokio::net::UnixListener` and is served directly with
  `axum::serve`, which supports Unix listeners natively (R34). `axum` implements
  `tower::Service`, so `tower-http` middleware (auth extraction, request limits,
  tracing) composes without glue. Both routers call the same typed application
  services through the transaction coordinator (KTD8); neither handler layer
  touches storage directly. Resolves OQ2's HTTP question. Rationale: fnox
  already depends on `rustls`; one TLS stack across client and server.
  (Research: axum 0.8 is Tokio-team maintained; `axum-server` is an independent
  community-maintained TLS adapter and is treated as such in dependency review —
  Unix-listener support is axum's own, not `axum-server`'s; actix-web's
...
- KTD2. **Storage: `redb` embedded ACID store, gated by a proof spike.**
  ...
  on redb, the store-facts everything above depends on: multi-table atomic
  state-plus-audit commit, crash recovery at injected kill points, a logical
  consistent snapshot for backup (KTD17), file growth and compaction behavior,
  and durable commit latency on the named low-cost reference host — with
  quantitative pass/fail thresholds recorded before higher units begin. If the
  spike fails, the SQLite fallback triggers and the swap is recorded here —
  before higher units are shaped around the store. (Narrowed 2026-07-16,
  wayfinder T04: bounded-executor backpressure and queue-saturation behavior are
  executor properties, identical over any store, and moved to U2/U6 executor
  tests; rotation-status query latency left the gate with KTD16's deferral —
  v0.1 status scans are covered by U6/U7 tests instead.) **Load-bearing
  caveat:** redb's single-writer lock is silently skipped on filesystems without
  file-lock support, permitting multi-process corruption — so startup takes its
  own explicit OS-level exclusive lock (`fd-lock` on a lockfile in the data
...
- KTD3. **Encryption at rest: an `age`-wrapped fixed-purpose keyring plus
  ...
  The audit-index key is minted at `init` like the others (32 random bytes;
  keeps the keyring format stable) but stays **dormant in v0.1** — KTD16's index
  is deferred, so the key exists unused until the index arrives, which then
  needs no keyring migration (wayfinder T10). The keyring plaintext contains the
  store id, keyring format version, keyring generation, purpose-key records, and
  recipient metadata; its `age` v0.12-encrypted envelope (to the boot-supplied
  server recipient, and optionally one documented offline recovery recipient) is
  stored as one opaque value in the redb `system_keyring` table — not a sidecar
  file — so keyring changes commit atomically with their audit events (KTD8). At
  boot the database is opened, the envelope is read and decrypted exactly once
  into `secrecy` types, and its embedded store id is compared with the clear
  store id before any other record is accepted — a transplanted envelope is
  rejected before readiness. Stored records use XChaCha20-Poly1305 (RustCrypto
  `chacha20poly1305`, NCC-audited) with associated data binding each ciphertext
  to store id, record type, mount, canonical path, secret version, and key id —
  transplanted ciphertext fails authentication. The cipher is identified by a
  fixed `cipher_suite_id` in the record header and is not operator-configurable;
  adding another suite is a storage-format migration, not a configuration
  toggle. What this buys over per-blob `age`: recipient rotation becomes an
  atomic keyring rewrap instead of a full-store re-encrypt, an offline recovery
  recipient costs nothing per record, per-record overhead drops, and purpose
  separation keeps token verification, metadata integrity, and audit
  confidentiality off the secret-encryption key. Record-key rotation (the rare
  full pass) is owned by U8. Recipients stay out of the authorization path per
  the Product Contract; `age` remains the only asymmetric mechanism and the seal
  boundary. The `age` crate is pre-1.0, so the exact version is pinned and
  decrypt fixtures survive upgrades. **Nonce lifecycle:** every AEAD encryption
  draws a fresh 192-bit CSPRNG nonce (the extended XChaCha nonce) stored in the
  record header; every rewrite and re-encryption draws a new nonce even when
  plaintext is unchanged; deterministic or counter-derived nonce construction
  remains prohibited. At 192 bits, accidental random collision is negligible at
  any plausible scale, so correctness and readiness do not depend on a
  collision-budget counter — the keyring may retain an informational per-key
  operation count for `doctor`. **Canonical cryptographic encoding:** record
  headers and AEAD associated data use a fixed binary format — magic bytes,
  format version, cipher-suite id, store id, record-domain id, key id,
  length-prefixed mount and canonical path segments, logical record id, and
  secret version where applicable. No JSON, debug formatting, string
  concatenation, platform endianness, or map iteration order enters an
  authenticated encoding; checked-in test vectors freeze the encoding across
  releases (U2).

- KTD4. **Audit chain: per-entry hash chain (`blake3`) over encrypted payloads,
  anchored by externally signed `ed25519-dalek` checkpoints mirrored off host.**
  Resolves OQ4 (per-entry, not per-batch) and answers R14's trust-anchor gap: a
  re-chaining attacker with write access is caught because the off-host
  checkpoint pins a chain head the attacker cannot retroactively rewrite.
  (Research: this is the Certificate-Transparency / Trillian signed-tree-head
  pattern reduced to single-node scale; full Merkle inclusion-proof
  infrastructure would be over-building for v0.1.) Entry payloads are encrypted
  under the keyring's audit key (R22 confidentiality — paths and identity names
  reveal topology); the plaintext envelope carries only sequence, timestamp,
  previous hash, and ciphertext digest, so the chain verifies without
  decrypting. **Concrete delivery mechanism:** the daemon stores only checkpoint
  public keys and key-lineage metadata — never the private key, and it has no
  checkpoint scheduler (Product Contract: no in-daemon timers).
  `audit checkpoint prepare` commits an audited canonical descriptor (store id,
  audit epoch, sequence range, chain head, KTD15 state digest, timestamp,
  signing-key id, previous-checkpoint digest); the local CLI loads the private
  signing key from an approved credential channel for the duration of the
  operation, signs the descriptor, fsyncs the checkpoint file, zeroizes the key,
  and calls `audit checkpoint register`, which verifies the signature against
  the stored public key and records the checkpoint digest. A systemd timer or
  the operator invokes the command; a crash between steps leaves either an
  unsigned prepared descriptor or a signed-but-unregistered file — both
  recoverable and visible to `doctor`. Checkpoints form their own signed chain
  through `previous_checkpoint_digest`; signing-key rotation retains the old
  public key and records a trust transition, so old checkpoints never become
  unverifiable. Backup manifests (R32) use the same external signing flow under
  a distinct domain-separation label. The off-host copy step is operator-owned
  (rsync/cron/systemd path unit) and documented in `docs/operating.md` — the
  server speaks no S3 or webhooks in v0.1.
  ...
  removing the signing key from the long-running process. **Segmented retention
  — deferred post-v0.1 (wayfinder T08):** v0.1 keeps every audit event in the
  active database; R27's capacity thresholds are the guard, and the audit table
  is schema-versioned so segmentation arrives later as an R35 forward-only
  migration. The deferred design, recorded so it lands intact: audit storage
  segmented by bounded event count or encoded byte size while preserving one
  global hash sequence per audit epoch; `audit archive` exports a closed
  encrypted segment, `audit archive register` records its verified digest, and
  `audit prune` may replace event rows with the authenticated segment manifest
  only after the signed-checkpoint and retention (R23) predicates hold; redb
  compaction is an offline operation after pruning (lifecycle `compacting`,
  R35).
...
- KTD5. **Credentials: structured opaque strings with indexed accessors and
  ...
  store id, credential kind, listener audience, accessor, and issue epoch (R24).
  Unknown accessors run the same MAC path against a fixed dummy record with the
  parsed kind, audience, and epoch, so timing does not reveal existence. Argon2
  is dropped: it is a password KDF for low-entropy human secrets, and this
  system has none — on the per-request token path it would turn every
  `vault kv get` into a deliberate CPU burn and a DoS amplifier, and even for
  login-frequency secret_ids the secrets are server-generated at full entropy.
  It returns only if a human-chosen passphrase ever enters the product. Tokens
  are capability-thin references — identity id, expiry, revocation state, never
  a grant snapshot (R28). secret_ids support TTL and bounded use counts,
  decremented atomically with token issuance and audit, matching Vault's own
  AppRole accessor model. Credentials may carry an optional stable
  `consumer_instance_id` — workload secret_ids normally require one, and tokens
  minted from a secret_id inherit it; it is non-secret metadata,
  MAC-authenticated under R39, and recorded in audit events (R13, R33).
  Satisfies R24 and R9's entropy floor without making authentication the
  throughput ceiling.
...
- KTD8. **One durability domain, one transaction coordinator, and one bounded
  ...
  transactions in an explicit order, performs the synchronous database and fsync
  work, and responds only after commit — redb permits one writer at a time, and
  this makes that ceiling explicit instead of blocking async workers on writer
  acquisition. The executor has separate bounded admission lanes: a data-plane
  lane and a small reserved control/recovery lane; queue saturation rejects new
  data-plane work before decryption with a stable overload error and bounded
  aggregate accounting (R13), while checkpoint preparation, backup, diagnostics,
  and orderly shutdown retain admission through the reserved lane (R27).
  Cancellation semantics are explicit: cancellation before a command begins
  prevents execution; once a transaction begins, the coordinator completes or
  aborts by the transaction result even if the client disconnects, and the audit
  outcome records committed state, never inferred response delivery (R26) — CAS
  remains the protection against ambiguous client retries. Mutations (KV
  write/delete/destroy, identity/grant/credential/rotation lifecycle, and
  keyring-envelope changes — recipient rewrap, keyring-generation and
  verification-key changes update the `system_keyring` record and append their
  audit event in the same transaction, KTD3) commit the state change and its
  audit entry in one multi-table write transaction; the response is sent only
  after commit. Reads revalidate the token and current grants inside the
  transaction boundary, decrypt, prepare the bounded response, append the read
  audit entry (with served version), commit, and only then transmit. A separate
  audit file, an async audit queue, and best-effort dual-writes are rejected:
  each reintroduces the crash window between state and record that R26 exists to
  close. No handler, middleware, background task, or application service owns a
...
- KTD9. **Authorization consumes a canonical `Resource` and a fixed capability
  enum.** An outer `RawTargetGuard` first validates `Request::uri()` before Axum
  route extraction — Axum's `Path` extractor percent-decodes before handlers
  run, so validation after extraction cannot tell a literal separator from an
  encoded alias. The guard rejects malformed escapes, percent-encoded separators
  and backslashes, encoded percent signs that could enable a second decode,
  encoded or literal dot segments, empty or repeated segments, control
  characters, NUL, overlong targets, duplicate security-sensitive query
  parameters, and noncanonical alternate encodings; it then performs exactly one
  decode and constructs an `EndpointRequest` carrying the endpoint kind and
  canonical `Resource`. That typed request is placed in request extensions and
  is the only resource representation available to authorization and handlers;
  handlers never reconstruct a resource from a framework-decoded path value, and
  the project uses no path-normalization middleware. One parser converts every
  endpoint form to `{mount, canonical_segments}` exactly once, rejecting
  ambiguous encodings (R8) — the raw validator and canonical decoder are one
  module with one test corpus. Logical path segments compare byte-for-byte after
  one validated UTF-8 decode — no Unicode normalization — and the supported
  segment character contract is published and frozen by U0/U11. Grants are
  `{mount, exact_or_subtree, prefix_segments, capabilities}` records over the
  closed capability set (R22). Read-current and read-history are distinct
  capabilities. No globs, no DSL. Authorization code accepts only the typed
  resource — never a raw URI string.
...
- KTD11. **Per-secret metadata is a first-class, schema-versioned record**
  ...
  (R39, KTD15) and its changes audited — not encrypted: logical paths, identity
  names, grants, and version metadata are readable to whoever holds the database
  file, the deliberate offline boundary stated in R4 and the Threat Model (audit
  payloads stay encrypted, KTD4, because they reveal access patterns rather than
  mere topology; sensitive operator notes such as runbooks and upstream account
  identifiers belong in a future local-only encrypted record, not in
  remote-API-visible `custom_metadata`) — and listing/age/status queries never
  decrypt value blobs. Reserved so post-v0.1 consumer declarations do not force
  a ciphertext format break.
...
- KTD13. **License: MIT**, entire tree, no `ee/` carve-out, nothing held back
  (R19). (Revised 2026-07-16, wayfinder T13 — Robert's call, the walk's first
  plan-decision override; formerly MPL-2.0. The repo's LICENSE file — MIT, ©
  2026 Robert Guss — stands. Consequence accepted knowingly: closed forks are
  permitted; the anti-BUSL posture is expressed by the product's own openness,
  not by copyleft.)
...
- KTD14. **Compatibility matrix:** U0 regenerates the initial matrix at
  ...
  retained only when it represents a documented deployed consumer or captures a
  materially different request contract. Exact versions, artifact checksums,
...
- KTD15. **Authenticated clear-state records and checkpointed state roots.**
  Clear structural records use a fixed binary encoding and a keyed MAC under the
  keyring's metadata-integrity key. The MAC domain includes store id, table id,
  primary key, schema version, record generation, and the canonical value bytes
  — so an offline editor who can write the database but not read the keyring can
  neither alter a grant nor transplant a valid record to another key or store.
  Per-record MACs alone cannot detect deletion or replacement with an older,
  formerly valid record; checkpoint creation therefore computes a deterministic
  digest over sorted `(table, key, generation, mac)` tuples from one consistent
  read snapshot and signs that digest together with the audit head (KTD4).
  Per-record verification is mandatory on use; full-state verification runs in
  `doctor --full`, backup verification, restore, migration, and checkpoint
  preparation (R39).

- KTD16. **Blind audit query index — deferred post-v0.1 (wayfinder T08).**
  Performance machinery, not security machinery: every hard part of this design
  (verify-against-primary, fail-closed health, rebuild) exists to make the
  optimization safe, so deferring the optimization removes the scaffolding too.
  In v0.1, rotation-status queries scan and decrypt primary audit events inside
  R23's bounded lookback window — redb reads run beside the single writer,
  decrypts are microsecond-scale, and the caller is an operator command with
  human latency tolerance. The primary events are always authoritative, so R33's
  former index-health closeout clause is gone and status can never be wrong for
  index reasons. Upgrade trigger: a measured status scan that actually hurts;
  append-only data means the index arrives via `audit index rebuild` with no
  migration crisis (the audit-index purpose key is already minted, dormant —
  KTD3). The deferred design, recorded so it lands intact: in the same
  transaction the coordinator writes index rows under keyed, domain-separated
  tags (audit-index key, KTD3) derived from canonical path, identity id,
  ...
  projection. Every query candidate is checked against its primary event —
  decrypt, authenticate, confirm path/version/actor, recompute the expected tags
  — so an index row can never prove adoption by itself; missing or corrupt index
  data may under-report observation but can never produce a positive adoption
  result, and `audit index rebuild` recreates the index into a temporary table
  and atomically swaps it after full verification. The index necessarily reveals
  equality and bucket cardinality among opaque tags to an offline database
  holder — that leak re-enters the Threat Model's metadata boundary if and when
  the index ships.

- KTD17. **Logical backup format and explicit recovery epochs.** The backup
  format is owned by the application and versioned independently from redb: one
  consistent read transaction (redb reads run concurrently with the writer)
  produces versioned canonical frames — never a copy of the live database file
  as the normative format — and restore always builds a new database, verifying
  every frame, record MAC, ciphertext, and audit relationship before atomically
  installing it (R32). A normal restore continues the existing audit epoch only
  when its head is consistent with the supplied checkpoint set; a rollback
  recovery — restoring behind the newest independently retained checkpoint —
  creates a new audit epoch and credential epoch and records the fork
  permanently (R32). This also keeps release-fixture restores stable across redb
  file-format changes (R35, R38).
...
- **Measured envelope, not a promised number:** the expected shape is tens of
  ...
  expressed as headroom over the documented expected peak plus p50/p95/p99 queue
  and commit latency. Correctness and fail-closed behavior outrank throughput at
  every margin.
- **Batch-ready, not batch-dependent:** the executor protocol (KTD8) may later
  ...
  transaction, but v0.1 ships with batching disabled; any batching must preserve
  per-command linearization, audit ordering, response-after-commit, bounded
  secret retention, and revocation ordering.
...
- The deployment host provides an approved boot credential channel for the
  ...
  directory, or attended TTY — environment-variable delivery is development-only
  behind an explicit flag (R37). The checkpoint private signing key is not a
  `serve` input: it is supplied only to explicit CLI checkpoint and
  backup-signing invocations (KTD4). Each workload's AppRole secret_id is
  delivered by its deployment mechanism. v0.1 implements no unattended-restart
  key custodian beyond the systemd-credentials path (Dependencies /
  Assumptions).
...
- **Clock model:** the wall clock is trusted for token expiry (R9), rotation age
  ...
  data-plane admission (readiness false) and requires an audited local override
  — the anomalous observation is never automatically persisted as the new
  high-water mark, so a rolled-back clock cannot silently extend bearer
  credentials and an NTP or virtualization fault cannot poison the store into a
  durable startup lockout. Startup refuses service when the clock sits behind
  the persisted mark beyond tolerance, and likewise refuses a persisted mark
  implausibly ahead of the current clock (R18). `clock repair` is the offline
  recovery for a mark known to be wrong: it requires the exact old mark, a
  replacement time, and an audited reason, and it increments the credential
  epoch before the store returns to `ready`, because prior TTL conclusions can
  no longer be trusted. v0.1 does not attempt to fix operator clocks, and
  operators running without NTP accept that TTL enforcement tracks their clock.
  No monotonic-only TTLs — they do not survive restart usefully.
...
- **M0 — Contract freeze:** U0 complete; U2's storage spike passes; the
  authenticated clear-record, crypto-header, audit-event, and logical-backup
  formats are reviewed and frozen (G0–G3; the audit query-index format left G2
  with KTD16's deferral).
- **M1 — Security kernel vertical slice:** `init → serve → fnox read/write` over
  TLS through audience-bound credentials, canonical authorization, authenticated
  state, atomic audit, and bounded storage admission — a real fnox read
  end-to-end at the close of U5. A development milestone, not a release.
- **M2 — Recovery-ready alpha:** recipient rewrap, the credential-epoch incident
  command (R41), logical backup, `backup verify --full`, restore, recovery audit
...
- **Approach:** Resolve and pin the current matrix at execution time (KTD14 —
  ...
  `vault` CLI or speak HTTP itself?), retry behavior, and status/error handling
  — the captured contract records the exact `vault` subcommands fnox invokes as
  well as the HTTP requests those CLI versions emit. Fixture normalization
  removes tokens, credential-command output, TLS private material, host paths,
  usernames, and nondeterministic identifiers before any artifact is committed;
  the secret-output canary scan (U11) covers U0 fixtures. Commit normalized
...
- **Exit gate:** R1's endpoint matrix, AE8's documented sequence, and U5/U11's
  ...
  before U5's router is implemented. Clean-host onboarding follows the BUSL exit
  ladder (Scope Boundaries; revised 2026-07-16, wayfinder T03), cheapest rung
  first: (1) point fnox at the `bao` CLI — U0's capture proves or kills this
  rung; (2) an upstream direct-HTTP fnox provider (confirmed welcome, fnox
  discussion #615); (3) document installing the HashiCorp Vault CLI with an
  honest licensing note — never describing the client stack as fully
  OSI-licensed while that is the only supported path. No shim is a v0.1 release
  gate; it is post-v0.1 and evidence-gated (Scope Boundaries). **Done-when:** U0
  is done when its named questions are answered with committed evidence — which
  endpoints each pinned client hits, how tokens and TLS roots are supplied,
  whether a fresh host runs fnox without the BUSL `vault` binary — not when the
...
- **Files:** `src/store/mod.rs`, `src/store/crypto.rs`, `src/store/keyring.rs`,
  `tests/store.rs`.
- **Approach:** Opens with the **KTD2 proof spike**, narrowed to store-facts
  (wayfinder T04): demonstrate on redb the multi-table atomic commit, kill-point
  crash recovery, a logical consistent snapshot (KTD17), file growth and
  ...
  fallback triggers here or never. (Bounded-executor backpressure and
  queue-saturation behavior are executor properties, not store properties — they
  are proven by executor tests here and in U6, not by the spike.) Then: a `meta`
  table (store id, `format_version`, `lifecycle` per R35); a `system_keyring`
  table holding the `age` envelope (KTD3); a `secret_meta` table per KTD11; a
  `secrets` table keyed by logical path holding versioned records (version
  number, created_time, key id, nonce, ciphertext). The server `age` identity
  loads into a `secrecy::SecretBox` at boot; the database is opened, the
  `system_keyring` envelope read and decrypted exactly once, and its embedded
  store id checked against the clear store id (KTD3; R21 — the identity is never
  written to disk, shell history, or argv). Values are AEAD-encrypted
  (XChaCha20-Poly1305, KTD3) under the keyring's record key with the canonical
  binary header and associated data binding store id, record type, mount, path,
  version, and key id, every encryption drawing a fresh 192-bit CSPRNG nonce
  stored in the record header — rewrites included; clear structural records
  carry KTD15 metadata-integrity MACs verified on every read (R39). The `meta`
  table also carries the wall-clock high-water mark (Assumptions — clock model).
  Encrypt on write, decrypt on read; plaintext buffers zeroize on drop.
  Checked-in cross-version crypto fixtures and canonical AAD test vectors freeze
  the encoding before storage-format fixtures freeze; the spike records XChaCha
  software performance on the supported targets — no configurable cipher
  negotiation exists.
...
- **Test scenarios:**
  ...
  - Fixed record-header and AAD test vectors decode identically across supported
  ...
  - Modify an identity, grant, credential record, or secret metadata record
    directly in the database → the record MAC fails and readiness becomes false
    (R39).
  - Copy a valid clear record and its MAC to another primary key or store → MAC
    verification fails (KTD15).
  ...
  - Saturate the storage executor's bounded queue → memory stays bounded, excess
    requests are rejected in order, and the reserved control lane stays live
    (KTD8 — executor test, moved from the KTD2 spike, wayfinder T04).
...
- **Test scenarios:**
  ...
  - The same logical path sent under a noncanonical percent encoding → rejected
    rather than authorized as an alias (KTD9).
  - Duplicate JSON keys or duplicate `version` parameters → rejected before any
    state lookup or mutation (KTD9).
...
- **Approach:** Implement `GET/POST /v1/:mount/data/:path`
  ...
  cover total encoded bytes, field count, field-name length, nesting depth, and
  list-result cardinality (R27). Errors use `{"errors":[...]}` with status
  ...
  **Vertical slice exit:** `init` → `serve` over TLS → the pinned fnox binary
  resolves a secret end-to-end through current-state authorization and atomic
  audit.
...
- **Goal:** The single commit boundary through which every state change and
  ...
  externally signed, registered checkpoints, that never records secret material
  and fails closed on write failure.
...
- **Approach:** The coordinator (KTD8) is built first: mutations commit state
  ...
  event — event id, request id, identity, credential accessor, consumer-instance
  id, auth method, canonical resource, requested capability, operation, outcome,
  reason code, effective time, and served/written version (R13) — never values
  or credential material (R25 extends this ban to all server output: logs,
  traces, errors, panics); entry payloads are encrypted under the keyring's
  audit key with a plaintext envelope of sequence, timestamp, previous hash, and
  ciphertext digest, so the `blake3` chain verifies without decryption (KTD4).
  Status and observation queries scan primary events inside R23's lookback
  window — KTD16's index and its same-transaction rows are deferred post-v0.1,
  so the scan path is the only query path and the primary events are always
  authoritative. Rate-limiter-dropped requests aggregate into bounded entries
  (R13). Checkpointing is external (KTD4): `audit checkpoint prepare` commits
  the canonical descriptor including the KTD15 state digest, the CLI signs it
  with the operator-supplied key and registers it via
  `audit checkpoint register`, and
  `audit verify [--checkpoint <path>] [--state]` checks the chain and state
  digest and reports R14's two v0.1 verification tiers. (`audit archive` /
  `audit archive register` / `audit prune` left v0.1 with segmented retention's
  deferral — KTD4.) Audit-log capacity is monitored with warning and
  stop-admitting thresholds and R27's preallocated `recovery.reserve` allowance
  (request-size and attempt-rate limits on the unauthenticated HTTP surface live
  in U4). The bounded executor's backpressure and queue-saturation behavior are
  proven here by executor tests (moved from KTD2's spike — wayfinder T04):
  saturate the request queue and assert bounded memory, orderly rejection, and a
  live control lane.
...
- **Test scenarios:**
  ...
  - Status-query scan over R23's lookback window returns correct
    on-current/on-prior/silent-since-write buckets computed from primary events,
    at a synthetic event volume representing years of small-team use, within
    operator-command latency tolerance (R33, R23 — the measured anchor for
    KTD16's upgrade trigger).
  - Saturate the bounded executor's request queue → memory stays bounded, excess
    requests are rejected in order, and the reserved control lane still serves
    backup/checkpoint/shutdown (KTD8 — moved from KTD2's spike, wayfinder T04).
  - Fill the active audit area to the stop-admitting threshold → data-plane
    admission stops while checkpoint registration, diagnostics, and orderly
    shutdown still succeed via the reserved lane and reserve file (R27, KTD8).
...
- **Files:** `src/consumer.rs`, `src/rotation.rs`, `src/api/kv.rs`,
  `tests/rotation.rs`.
- **Approach:** The declared-consumer registry (R40) is local-control-plane CRUD
  over MAC-authenticated records (R39). The consumer view reports declared,
  authorized, and observed as three separately labeled sets with reconciliation
  states (declared-but-unauthorized, authorized-but-undeclared,
  declared-but-unobserved, observed-but-undeclared); observed reads — last-read
  time and version — come from scanning primary audit events over R23's window
  (KTD16's index is deferred post-v0.1), and recency annotates, never filters
  (R10). `rotation begin` creates a prepared record snapshotting the declared,
  authorized, and active consumer-instance sets, the prior version, the expected
  CAS, actor, and time — without writing a secret. `rotation cutover` accepts
  the bounded secret object through an approved input channel (R37) and
  atomically performs the CAS write plus the prepared → cutover transition
  (R11). States are
  `prepared | cutover | completed | cancelled-before-cutover | superseded` — no
  post-cutover abort; recovery after cutover is an explicit copy-forward
  rollback to a new higher version. `rotation status <path>` classifies each
  snapshotted consumer as `on-current` / `on-prior` / `silent-since-write` from
  R13's versioned read events, scanned in the primary store over R23's window —
  at declared-consumer, identity, and consumer-instance granularity, with
  accessor drill-down where distinct accessors appear, labeled "fetched", never
  "using" (R33). `rotation complete` succeeds only while the target is still the
  current version and every snapshotted consumer is `on-current` — retired
  instances need a recorded reason — otherwise it requires
  ...
  registry mutation, and audit read each require their capability (R22), and all
  mutations flow through the coordinator (KTD8).
- **Test scenarios:**
  ...
  - Covers AE13. A declared-but-unmigrated integration, an authorized-but-silent
    workload, and an observed-but-undeclared identity all stay separately
    visible in the consumer view; the declared consumer enters the rotation
    snapshot and blocks unqualified completion (R40).
  - Cancellation before cutover leaves the secret unwritten and closes the
    record `cancelled-before-cutover`; no cancellation path exists after cutover
    (R11).
  ...
  - Two instances under one identity: one fetches the new version, one keeps
    ...
    expired instance shows `retired-without-proof` and requires a reason or the
    override (R13, R33).
...
- **Approach:** Distinct offline CLI operations (`age` has no rotation
  ...
  the new envelope, incremented keyring generation, and audit event in one redb
  transaction (KTD3, KTD8): no record is touched, and a crash exposes either the
  old generation with its old audit head or the new generation with its matching
  audit event — never a cross-file partial state. **Record-key rotation** — the
  rare case (key-compromise response, crypto hygiene) — runs offline only: the
  CLI sets `lifecycle=reencrypting` (U1's `serve` refuses to start), streams
  every record into a new store file re-encrypted under a new record key with
  fresh nonces and associated data (KTD3 — never a reused nonce, even for
  unchanged plaintext), fsyncs, atomically renames over the old file, and
  restores `lifecycle=ready`. An interrupted pass leaves the original store
  untouched and functional — delete the partial `.new` file and rerun; no
  resumable-cursor machinery, no mixed-encryption state to reason about. Purpose
  keys have deliberately different rotation semantics, fixed here so no generic
  "rotate all keys" command can promise the impossible: the secret-record key
  rotates by the offline whole-store re-encryption; the metadata-integrity key
  by an offline rewrite and MAC regeneration; the audit-payload key
  forward-only, retaining decrypt-only historical keys because rewriting
  ciphertext would invalidate anchored audit hashes; and the credential-verifier
  key only together with a credential-epoch increment (R41), invalidating all
  outstanding tokens and secret_ids. (The fifth semantic — the audit-index key,
  rotated by rebuilding the non-authoritative index from primary events — leaves
  v0.1 with KTD16's deferral; the key stays minted-but-dormant in the keyring,
  KTD3, wayfinder T10.) Audit history survives every operation.
...
- **Approach:** `axum-server` `bind_rustls` with PEM cert/key from config,
  ...
  header are marked sensitive (`tower-http` sensitive-headers) before tracing is
  invoked (KTD9, R25). Reverse-proxy mode is an explicit listener type, not a
  boolean relaxation: it binds only a configured Unix socket or loopback
  address, forwarded source headers are accepted only from the exact configured
  proxy peer in one allowlisted syntax — otherwise ignored — and rate-limit
  source buckets use the verified proxy-derived source only in this mode (R20,
  R27). Graceful shutdown wires here: stop admission, drain, flush, final
  shutdown event, zeroize (R36).
- **Test scenarios:**
...
- **Approach:** `backup` produces KTD17's logical archive from one consistent
  redb read transaction (U2's proven snapshot mechanism — never a raw `cp` of a
  live B-tree, and never the redb file as the normative format): versioned
  ...
  recorded, and a signed manifest (store id, schema version, keyring generation,
  chain head, latest included checkpoint, per-table counts and digests),
  encrypted to one or more configured backup recipients — which may differ from
  the live server recipient. Private keys never enter the archive (R32); the
  documented recovery package adds the offline `age` identity and checkpoint-key
  copies. `restore` builds a new database under `lifecycle=restoring` in a
  temporary sibling of the empty target directory, verifies every frame, record
  MAC, ciphertext, audit chain, and checkpoint relationship, fsyncs, and
  atomically installs it only after all verification succeeds; it increments the
  credential epoch invalidating all pre-restore tokens and secret_ids, and
  records the restore as a recovery event in the first new audit entry (R32).
  `backup verify` checks an archive without restoring it; `backup verify --full`
  is R32's rehearsal — restore into an isolated temporary directory, validate
  schema and tables, authenticate and decrypt every encrypted record without
  emitting plaintext, verify chain and checkpoint relationship, tear the
  temporary store down, and record the success so `doctor` can report rehearsal
  age.
- **Test scenarios:**
...
- **Approach:** The harness starts a server instance and drives it with the
  ...
  no log, trace, panic, or test artifact (R25). A benchmark records the
  audited-read/write baseline on a named reference host (Performance and
...
- **Approach:** The CLI's command families are documented as a coherent surface
  ...
  `credential epoch rotate`, `clock repair`, `backup`, `restore` — each with
  human-readable and stable JSON output (R16), secret input never via argv
  (R37), and destructive commands requiring exact identifiers plus an audited
  reason. `doctor` checks what an operator would otherwise learn at 2am:
  ...
  the last successful `backup verify --full` rehearsal (R32), audit-table size
  against KTD16's upgrade trigger (the index-health check arrives with the
  index, post-v0.1), informational per-key operation counts (KTD3), TLS cert
  expiry, and clock sanity against the persisted high-water mark — without
  touching secret values, with stable JSON and documented stable exit codes.
  ...
  signing key only to the short-lived checkpoint CLI process (KTD4) — exercised
  by U1's credentials-directory test. MIT at the tree root with no `ee/`
  carve-out (R19, KTD13 — revised from MPL-2.0, wayfinder T13). `deny.toml`
  configures `cargo-deny` for licenses and advisories (license config follows
  MIT) so the Verification Contract's `cargo deny check` gate has an owner. The
  README leads with the positioning paragraph settled at T13 (working name until
  OQ3's deadline):

  > **[name]** is a single-binary, Vault-KV-v2-compatible secrets server for
  > teams too small to run Vault. It exists for one workflow: rotating a
  > credential without an outage. Every read is audited atomically, so before
  > you revoke anything upstream the server can show you exactly who fetched the
  > old value and who has picked up the new one — declared, authorized, and
  > observed consumers, never collapsed into one comfortable list. The ops-light
  > claim is a list of refusals: no cluster, no external database, no policy
  > language, no plugin system, no remotely reachable management surface, no
  > unseal ceremony. Management lives on a local socket; one operator can run
  > backup, restore, key rotation, and incident response alone. Works with
  > unmodified `vault`/`bao` CLIs and fnox. MIT, whole tree, nothing held back.

  The README also says "outgrow it → OpenBao" out loud (T01's accepted
  consequence). Release integrity (R38) lands here: the release pipeline emits
  signed checksums, an SBOM, and build provenance for every artifact; MSRV is
  pinned and `Cargo.lock` is committed with `--locked` enforced everywhere;
  `SECURITY.md` documents the vulnerability-reporting process; every
  storage-format change ships upgrade notes; and the release smoke test consumes
  the actual published binary, restoring at least one prior release's store
  fixture (R35).
...
- Fail-closed postures hold: unsafe-config startup refusal (R18, clock rollback
  ...
  readiness false (R39), restore credential epoch (R32), init
...
- **Audit scale machinery** (deferred 2026-07-16, wayfinder T08): KTD16's blind
  query index and KTD4's segmented retention/archive/prune. v0.1 scans primary
  events inside R23's window and retains all events; both designs are recorded
  in their KTDs so they land intact when a measured status scan or
  `doctor`-reported capacity pressure triggers them. The index arrives via
  `audit index rebuild` over append-only data (its purpose key is minted,
  dormant — KTD3); segmentation arrives as an R35 forward-only migration.
- **Vault-CLI shim** (demoted 2026-07-16, wayfinder T03): post-v0.1,
  evidence-gated behind the BUSL exit ladder (Scope Boundaries) — built only if
  `bao`, the upstream fnox provider (confirmed welcome, fnox discussion #615),
  and a documented `vault` install all fail AND someone is actually blocked.
